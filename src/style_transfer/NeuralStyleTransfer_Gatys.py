...
class GramMatrix(nn.Module):
    def forward(self, input):
        ...
        G = torch.bmm(F, F.transpose(1,2))   # Performs a batch matrix-matrix product of matrices stored. batch1 and batch2 must be 3-D tensors each containing the same number of matrices.
        G.div_(h*w)
        return G
